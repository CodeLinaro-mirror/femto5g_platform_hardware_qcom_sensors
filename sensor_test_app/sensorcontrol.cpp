        ...
        for (int ii = 0; ii < n; ii++) {
            ...
            */
            ...
            if (buffer[ii].type == SENSOR_TYPE_META_DATA) {
                printf("INFO Flush complete, sensor:%d, what:%d\n",
            ...
            float deltaT = float(
                buffer[ii].timestamp - sensorStatsList[sensorIndex].timestamp) / 1000000.f;
            sensorStatsList.editItemAt(sensorIndex).timestamp = buffer[ii].timestamp;
            
            float deltaMono = (now_ns() - buffer[ii].timestamp) / 1000000.f;

            if (printOnScreen) {
                // sensor name
                printf("DATALOG %-30s, ",
                       sensorStatsList[sensorIndex].getName());
                // data values
                if (buffer[ii].type == SENSOR_TYPE_STEP_COUNTER) {
                    printf(
                        "%13" PRIu64 ", %13d, %13d, ",
		    ...
		    buffer[ii].type == SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED ||
                    buffer[ii].type == SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED) {
