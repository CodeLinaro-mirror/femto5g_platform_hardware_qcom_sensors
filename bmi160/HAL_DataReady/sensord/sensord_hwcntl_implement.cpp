...
//lint -esym(551, BMI160_acc_resl)
/*BMI160_acc_resl is used once configuration is changed*/
static float BMI160_acc_resl = 0.061; //16bit ADC, default range +-2000 mg. algorithm input requires "mg"
//lint -esym(551, BMA255_acc_resl)
/*BMA255_acc_resl is used once configuration is changed*/
    ...
    if(ACC_CHIP_BMI160 == accl_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_acc_open)
            {
                ...
                ret = wr_sysfs_oneint("acc_op_mode", acc_input_dir_name, SENSOR_PM_SUSPEND);
        ...
        }else
        {
            ...
            ret = wr_sysfs_oneint("acc_odr", acc_input_dir_name, odr_Hz);

            /*activate is included*/
            if (0 == is_acc_open)
            {
                ret = wr_sysfs_oneint("acc_op_mode", acc_input_dir_name, SENSOR_PM_NORMAL);
    ...
    if(GYR_CHIP_BMI160 == gyro_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_gyr_open)
            {
                ...
                /*BMI160 control nodes are in ACC input dir*/
                ret = wr_sysfs_oneint("gyro_op_mode", acc_input_dir_name, SENSOR_PM_SUSPEND);
        ...
        }else
        {
            ...
            /*BMI160 control nodes are in ACC input dir*/
            ret = wr_sysfs_oneint("gyro_odr", acc_input_dir_name, odr_Hz);

            /*activate is included*/
            if (0 == is_gyr_open)
            {
                /*BMI160 control nodes are in ACC input dir*/
                ret = wr_sysfs_oneint("gyro_op_mode", acc_input_dir_name, SENSOR_PM_NORMAL);
    ...
    if(MAG_CHIP_BMI160 == magn_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_mag_open)
            {
                ...
                ret = wr_sysfs_oneint("mag_op_mode", acc_input_dir_name, SENSOR_PM_SUSPEND);
        ...
        }else
        {
            ...
            ret = wr_sysfs_oneint("mag_odr", acc_input_dir_name, odr_Hz);

            /*activate is included*/
            if (0 == is_mag_open)
            {
                ret = wr_sysfs_oneint("mag_op_mode", acc_input_dir_name, SENSOR_PM_NORMAL);
...
static void ap_config_physensor(bsx_u32_t input_sid, bsx_f32_t sample_rate)
{
    int32_t ret = 0;

    if (BSX_PHYSICAL_SENSOR_ID_INVALID == input_sid)
    ...
    switch (input_sid)
    {
        ...
        case BSX_INPUT_ID_ANGULARRATE:
            ...
            break;
        default:
            PWARN("unknown input id: %d", input_sid);
            ret = 0;
            break;
    }
...
static void ap_send_config(int32_t bsx_list_inx)
{
...

    {
        ...
        {
            bsx_config_output[0].sample_rate = p_config[(long)(bsx_list_inx - list_inx_base)].delay_onchange_Hz;
        }else
        {
            CONVERT_DATARATE_CODE(p_config[(long)(bsx_list_inx - list_inx_base)].data_rate, bsx_config_output[0].sample_rate);
        }
...

    {
        switch (bsx_list_inx) {
            case SENSORLIST_INX_ACCELEROMETER:
                input_sid = BSX_INPUT_ID_ACCELERATION;
                break;
            case SENSORLIST_INX_MAGNETIC_FIELD_UNCALIBRATED:
                input_sid = BSX_INPUT_ID_MAGNETICFIELD;
                break;
            case SENSORLIST_INX_GYROSCOPE_UNCALIBRATED:
                input_sid = BSX_INPUT_ID_ANGULARRATE;
                break;
            default:
        ...
        ap_config_physensor(input_sid, bsx_config_output[0].sample_rate);
    }
...
static void ap_send_disable_config(int32_t bsx_list_inx)
{
...

    {
        ...
        ap_config_physensor(input_sid, SAMPLE_RATE_DISABLED);
    }
    return;
}
...
//lint -sem(BstSimpleList::list_add_rear, custodial(1))  -e593
/*p_hwdata is handled by BstSimpleList::list_add_rear and freed by another thread*/
static void ap_hw_poll_bmi160_acc(BstSimpleList& dest_list)
{
    int32_t ret;
    ...
    while( (ret = read(acc_input_fd, event, sizeof(event))) > 0)
    {
        ...
        p_hwdata->x = event[2].value * BMI160_acc_resl;
        p_hwdata->y = event[3].value * BMI160_acc_resl;
        p_hwdata->z = event[4].value * BMI160_acc_resl;
...
static void ap_hw_poll_bmi160_gyr(BstSimpleList& dest_list)
{
...
static uint32_t IMU_hw_deliver_sensordata(BstSensor *bstsensor)
{
    ...
    for (j = 0; j < ARRAY_ELEMENTS(poll_fds); j++)
    {
        ...
        {
            case 0:
                ap_hw_poll_bmi160_acc(bstsensor->tmplist_hwcntl_acclraw);
                break;

            case 1:
                ap_hw_poll_bmi160_gyr(bstsensor->tmplist_hwcntl_gyroraw);
                break;
        }
    ...
    {
        (void)pthread_mutex_lock(&(bstsensor->shmem_hwcntl.mutex));

        ret = bstsensor->shmem_hwcntl.list.list_mount_rear(&(bstsensor->tmplist_hwcntl_acclraw));
        if(ret){
            PWARN("list mount fail");
        }

        ret = bstsensor->shmem_hwcntl.list.list_mount_rear(&(bstsensor->tmplist_hwcntl_gyroraw));
        if(ret){
            PWARN("list mount fail");
        }

        (void)pthread_cond_signal(&(bstsensor->shmem_hwcntl.cond));
        (void)pthread_mutex_unlock(&(bstsensor->shmem_hwcntl.mutex));
    }
#endif
    return 0;
...
static int32_t ap_hwcntl_init_ACC()
{
    ...
    if(ACC_CHIP_BMI160 == accl_chip)
    {
        ...
        switch(accl_range){
            case ACC_CHIP_RANGCONF_2G:
                ret += wr_sysfs_oneint("acc_range", acc_input_dir_name, BMI160_ACCEL_RANGE_2G);
                BMI160_acc_resl *= 1;
                break;
            case ACC_CHIP_RANGCONF_4G:
                ret += wr_sysfs_oneint("acc_range", acc_input_dir_name, BMI160_ACCEL_RANGE_4G);
                BMI160_acc_resl *= 2;
                break;
            case ACC_CHIP_RANGCONF_8G:
                ret += wr_sysfs_oneint("acc_range", acc_input_dir_name, BMI160_ACCEL_RANGE_8G);
                BMI160_acc_resl *= 4;
                break;
            case ACC_CHIP_RANGCONF_16G:
                ret += wr_sysfs_oneint("acc_range", acc_input_dir_name, BMI160_ACCEL_RANGE_16G);
                BMI160_acc_resl *= 8;
                break;
            default:
                break;
